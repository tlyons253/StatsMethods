...
dsr<-surv.dat%>%
  filter(time!='1')%>%
  group_by(id)%>%
  filter(cumsum(LD==0) <2)%>%
  ungroup()%>%
  summarize(dsr=sum(LD)/length(LD))

surv.dat%>%
  filter(time!='1')%>%
    mutate(time.int=time-1)%>%
glm(LD~ time.int,family='binomial',data=.)%>%
  broom::tidy(.)%>%
  select(estimate)->tmp


  data.frame(time=seq(1,10,1))%>%
               mutate(
...
dsr.df<-data.frame(time=seq(2,10,1),
                   dsr=dsr)%>%
  add_row(time=1,dsr=1)%>%
  arrange(time)%>%
...
The figure to the right demonstrates that. It comes from a TTE workshop I took at the TWS meeting in Reno in 2019, though the data are on a long-term deer survival study in northern MN. [You can access all the workshop materials here.](https://github.com/tlyons253/StatsMethods/tree/master/TTE/TWS2019)
...
This is the hazard at time $t$, is the product of some baseline hazard $\lambda_0$, and covariates $\lambda_t$that can be modeled using a log link 
...
Another way to write the Weibull hazard can be found in handout 9 [here]("https://stat.ethz.ch/education/semesters/ss2011/seminar/contents/handout_9.pdf") is as:
...
The model below a parametric model with an exponential distribution If you use a simple exponential function, your baseline hazard is constant and it is the same as a Cox PH model that doesn't assume a baseline hazard.
...
true.mat%>%
  ...
  pivot_longer(cols=starts_with('V'),
               ...
               values_to = 'ld')%>%
  mutate(
    is.obs = rbinom(n = nrow(.), size = 1, prob = 0.6),
    is.obs = ifelse(visit == 'V25', 1, is.obs))%>%
  mutate(age=as.numeric(str_replace(visit,'V','')))%>%
  group_by(nest.id)%>%
  mutate(y=case_when(ld==0& row_number()!=min(
    which(
      ld==0 &is.obs==1))~NA,
    is.obs==0~NA,
    T~ld))%>%
  filter(any(y==1))%>% #kicks out any nests that failed before discovery
  ungroup()%>%
  select(-visit,-ld,-is.obs)->eh.long


eh.long%>%
  group_by(nest.id)%>%
  filter(row_number()==min(which(y==1),na.rm=TRUE)|
           row_number()==min(which(y==0),na.rm=TRUE)|
           row_number()==max(which(y==1)),na.rm=TRUE)%>%
  mutate(right=lead(age),
         censor=lead(y))%>%
  filter(!(is.na(right)))%>%
  select(nest.id,left=age,right,censor)->tte.dat

eh.long%>%
  filter(!is.na(y))%>%
  group_by(nest.id)%>%
  mutate(exposure=age-lag(age))%>%
  ungroup()%>%
  filter(!is.na(exposure))->glm.dat

#save(tte.dat,glm.dat,eh.long,true.mat,file='./objects/simdata_time.RData')
```

```{r, echo=FALSE}
load('./objects/simdata_time.RData')
```
...
MCMCvis::MCMCsummary(samples.time)->post.parms

#post.parms

#MCMCvis::MCMCtrace(samples.time,pdf=FALSE)



post.parms%>%
  rownames_to_column(var='parm')%>%
  filter(str_detect(parm,'UCH0'))%>%
  rename(med=`50%`)%>%
  select(parm,mean,med)%>%
  mutate(dsr.mean=1-mean,
        dsr.med=1-med,
        cumsurv.mean=cumprod(dsr.mean),
        cumsurv.med=cumprod(dsr.med))->sr.haz




 # save(.,file='./TTE/objects/timevar_results.RData')

```


In this case, this simple approach has issues. The data generating model has dsr near the end being >0.99, which is hard for the hazard model to estimate.
...
###########
glm.dat%>%
  mutate(age2=(age-exposure))->glm.dat

mod.logexp<-glm(y~age2,
                family=binomial(
                  link=logexp(glm.dat$exposure)),
                data=glm.dat)
...
It turns out that this isn't advisable when there are covariates like this. It may be because it violates the proportional hazards assumption by having time-varying survival like this, but I'm not totally sure to be honest. It could just be a function of the way cloglog cdf changes with covariate values.
...
glm.dat%>%
  ungroup()%>%
  mutate(age2=(age-exposure),
         c.age=scale(age,scale=FALSE),
         c.exp=scale(exposure),scale=FALSE)->glm.dat

glm.dat$exposure

mean(glm.dat$age)->mu.age
mean(glm.dat$exposure)


mod.clog<-glmmTMB::glmmTMB(y~age+exposure,
              family=binomial(link='cloglog'),
              data=glm.dat)
...
# We need the mean exposure value as an offset to get back to the data-generating value? Not here, it makes it very low
```
...
eh.long%>%
  pivot_wider(names_from=age,
              values_from=y)%>%
  column_to_rownames(var='nest.id')%>%
  as.matrix()->y

#Create vector of first obs
#create vector of last obs
...
time.ss<-nimble::nimbleCode({
for(i in 1:nests){
  for(t in (first[i]+1):last[i]){
    S[i,t]<-phi[t]*y[i,t-1]
    y[i,t]~dbern(S[i,t])

  logit(phi[t])<-mu.phi +beta*age[t-1]        
  }
...
nim.init<-function(){list(
  mu.phi=runif(1,-3,3),
  beta=runif(1,-1,1),
  S=matrix(runif(nrow(y)*25,0.5,0.999),nrow=nrow(y),ncol=25),
  y=matrix(rep(1,nrow(y)*25),nrow=nrow(y),ncol=25)
)
}


ss.nimmod<-nimble::nimbleModel(code=time.ss,
                               constants=nim.ss,
                               data=nim.dat,
                               inits=nim.init())

#throws an error essentially asking for the z matrix you include as data or initial values in CJS from the Kery Schaub BPA book

ss.nimmod$calculate()  # Check to make sure initial values are good.
                              #Should be a real number
ss.nimmod$initializeInfo()

ss.config<-nimble::configureMCMC(ss.nimmod)

# now is when you can specify different samplers and monitors
#time.config$addMonitors("","","")

ss.config$replaceSamplers(target = c('mu.phi','beta'),
                       type= 'AF_slice')

ss.build<-nimble::buildMCMC(ss.config)


#compile things

ss.comp<-nimble::compileNimble(ss.nimmod)

ss.mcmc<-nimble::compileNimble(ss.build, project=ss.comp)


samples.ss.time<-nimble::runMCMC(ss.mcmc,
                             niter=8000,
                             nburnin = 1000,
                             nchains = 3)
#more errors here but not fatal

MCMCvis::MCMCsummary(samples.ss.time)%>%
  pull(`50%`)->ss.parms

MCMCvis::MCMCtrace(samples.ss.time,pdf=FALSE)


age=seq(0,23,1)
ss.dsr<-plogis(ss.parms[2]+ss.parms[1]*age)
ss.dsr
...
data.frame(type=c('Hazard','Logistic exposure','State Space','"Truth"'),
           c.surv=c(sr.haz[24,6],prod(dsr.logexp),prod(ss.dsr), prod(S)))%>%
 mutate(c.surv=round(c.surv,digits=3))%>%
  rename(Model=1,
         `Cumulative survival\n(25d)`=2)%>%
  pivot_wider(names_from = Model,
              values_from = 2)%>%
  mutate(Parameter='25d survival')%>%
  relocate(Parameter)->time.tab

   flextable::flextable(time.tab)%>%
flextable::set_caption(flextable::as_paragraph(flextable::as_b('Table 4.'), 
                                    " Cumulative survival estimate comparison among methods; (time-varying survival)."),
                       align_with_table = FALSE)%>%
  flextable::theme_vanilla() %>% 
  flextable::align(align = "center", part = "all")%>%
  flextable::border_inner_h(
...
All estimates look good again. I will note, that there is considerably more error when comparing the linear model parameters ($\mu$;$\beta$), but as they say, it all comes out in the wash, or, sampling error. Remember, signs are parameters are reversed for the hazard model. 
